...
se_ui_handle* se_ui_handle_create(se_window* window, se_render_handle* render_handle, const se_ui_handle_params* params) {
    ...
    scene_params.objects_2d_count = params->elements_count * params->objects_per_element_count + 1; // at least one object for the ui
    scene_params.objects_3d_count = 0;
    scene_params.scenes_2d_count = params->elements_count * params->children_per_element_count + 1; // at least one scene for the ui, the parent scene
...
se_object_2d_ptr se_ui_element_add_object(se_ui_element* ui, const c8* fragment_shader_path) {
    ...
    se_vec2 scale = {1, 1};
    se_vec2 position = {0, 0};
    se_object_2d_ptr object_2d = se_object_2d_create(scene_handle, fragment_shader_path, &position, &scale, 0);
