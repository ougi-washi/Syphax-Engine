...
int main(void) {
	...
	se_window_handle window = se_window_create("Syphax - SDF Step 3", 1280, 720);
	...
	se_sdf_noise_set_frequency(noise, 2.0f);
	se_sdf_noise_set_offset(noise, &s_vec3(0.25f, 0.0f, 0.0f));
	...
	while (!se_window_should_close(window)) {
		...
		se_camera_set_window_aspect(camera, window);
		se_render_clear();
		se_sdf_render_to_window(scene, camera, window, 0.1f);
